...
class LearnerProfile(TimeStamped):
    ...
    # ระดับคำศัพท์ที่ยอมให้ระบบหยิบ *คำใหม่* มาสอน
    #
    # กรองเฉพาะคำใหม่ ไม่กรองของที่ถึงกำหนดทบทวน — ถ้ากรองทั้งหมด
    # การ์ดระดับที่ถูกตัดออกจะไม่มีวันถูกทบทวนอีกเลย แต่ยังนับเป็นของค้างอยู่
    # ผู้เรียนจะเห็นตัวเลขของค้างพอกขึ้นเรื่อยๆ โดยไม่มีทางเคลียร์ได้
    # เริ่มอะไรไปแล้วต้องเรียนให้จบ เปลี่ยนได้แค่ว่าจะ *เริ่ม* อะไรใหม่
    vocab_levels = models.JSONField(
        default=list, blank=True, verbose_name="ระดับคำศัพท์ที่จะเรียนใหม่",
        help_text="เช่น [4, 5] — ว่างแปลว่าทุกระดับ",
    )
