...
class Question(Provenance, TimeStamped):
    ...
    # บทที่ต้องอ่านออกเสียงสำหรับข้อฟัง — บทสนทนา/บทยาว ต่อท้ายด้วยคำถาม
    #
    # เก็บที่ระดับข้อ ไม่ใช่ที่ ItemGroup เพราะข้อ 31-45 ใช้บทเดียวกันแต่คนละคำถาม
    # และชุดฝึกรายวันสลับข้อจากหลายพาร์ทเข้าด้วยกัน (D4 interleaving)
    # ข้อจากบทเดียวกันจึงไม่ได้อยู่ติดกัน แต่ละข้อต้องฟังจบได้ด้วยตัวเอง
    #
    # ไม่เก็บไฟล์เสียง — เสียงสร้างที่เครื่องผู้เรียนด้วยตัวอ่านของเบราว์เซอร์
    # เหตุผลอยู่ใน core/listening.py
    audio_script = models.TextField(
        blank=True, verbose_name="บทที่อ่านออกเสียง",
        help_text="ข้อฟังเท่านั้น — ว่างแปลว่ายังไม่มีเสียง ข้อนั้นตอบไม่ได้",
    )
    # บทแยกเป็นช่วงพูด พร้อมบอกว่าใครพูด — ใช้เล่นเสียงสลับคนและเว้นจังหวะ
    # อ่านรวดเดียวด้วยเสียงเดียวฟังยากกว่าของจริง ไม่ใช่แค่ไม่เหมือน
    # (เหตุผลเต็มอยู่ที่ core/listening.speech_turns)
    audio_turns = models.JSONField(
        default=list, blank=True, verbose_name="ช่วงพูดในบท",
        help_text='[{"who": "f|m|q|n", "text": "…"}] — ว่างได้ ระบบจะกลับไปอ่านรวดเดียว',
    )
